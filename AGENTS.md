...
- keep real Agent Browser control under the explicit `browser session` / `browser.session.*` lifecycle (`open`, `observe`, action, `live`, `auth`, `close`). Provider session ids stay behind the hosted Worker boundary; user/agent responses should use Vibecodr `job_` session ids and screenshot proof artifacts. Live watch/takeover links must use Vibecodr-owned short-lived URLs; when `VC_TOOLS_BROWSER_HANDOFF_UI_BASE_URL` is configured, CLI/API responses should point users to the main Vibecodr `/agent-browser/live/:sessionId#token=...` route while this Worker remains the token/session/Live View authority. The web app must send handoff tokens to the Worker in `X-Vibecodr-Handoff-Token` with credentials omitted; the direct Worker HTML fallback may keep accepting `?token=` for compatibility, but JSON/HTML target, session, and error text must redact URL userinfo, sensitive query/hash tokens, bearer tokens, and credential assignments before display. The token-gated live page may take control, give control back, end the live link, or close the browser; the user-facing app should call the close behavior "End browser" rather than exposing provider lifecycle jargon. The legacy `/agent-browser/handoff/:sessionId#token=...` route is compatibility only. Do not accept or serialize raw cookies, auth headers, passwords, storage state, provider session ids, or Cloudflare Live View URLs in normal API/job output.
- Agent Browser live view means the actual Cloudflare Browser Run tab view for the live provider session, not a simulated preview. The Worker should mint and refresh real `live.browser.run` target URLs behind Vibecodr token-gated routes and let the web app frame that live view as the product surface. Inspector/DevTools Live View is opt-in only (`browser session live|auth --debug` or `--view devtools`) and must use the same token-gated URL path without exposing provider session ids.
